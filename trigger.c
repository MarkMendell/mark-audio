...
main(int argc, char **argv)
{
	...
	int fds[2];
	if (pipe(fds))
		die("trigger '%s': pipe: %s", cmd, strerror(errno));
	pid_t pid = fork();
	if (pid == -1)
		die("trigger '%s': fork: %s", cmd, strerror(errno));
	if (!pid) {  // child
		int nullfd = open("/dev/null", O_RDONLY);
		if (nullfd == -1)
			die("trigger '%s' child: open: %s", cmd, strerror(errno));
		int fd;
		if ((fd=0, dup2(nullfd, fd) == -1) || (fd=1, dup2(fds[1], fd) == -1))
			die("trigger '%s' child: dup2 %d: %s", cmd, fd,
				strerror(errno));
		if (close(fds[0]) || close(fds[1]) || close(nullfd))
			die("trigger '%s' child: close: %s", cmd, strerror(errno));
		if (execlp("sh", "sh", "-c", cmd, NULL))
			die("trigger '%s' child: execlp: %s", cmd, strerror(errno));
	}
	if (close(fds[1]))
		die("trigger '%s': close: %s", cmd, strerror(errno));
	FILE *child = fdopen(fds[0], "r");
	if (child == NULL)
		die("trigger '%s': fdopen: %s", cmd, strerror(errno));
	setbuf(child, NULL);
	...
	while ((len = getline(&line, &_, stdin)) != -1) {
		...
		do
			if (!isdigit(line[chari]))
				die("trigger '%s': lines must be 'index[\tcommand]', "
					"not: %s", cmd, line);
			else
				cmdi = 10*cmdi + (line[chari] - '0');
		while ((++chari < len) && (!isspace(line[chari])));
		if (cmdi < samplei)
			die("trigger '%s': command '%s' out of order", cmd, line);
		...
		while (samplei != cmdi) {
			...
			if (ferror(child))
				die("trigger '%s': fread: %s", cmd, strerror(errno));
			fwrite(buf, sizeof(sample), readc, stdout);
			if (ferror(stdout))
				die("trigger '%s': fwrite: %s", cmd, strerror(errno));
			if (feof(child))
				goto cleanup;
			samplei += readc;
		}
		...
		if (chari++ == len)  // no command
			...
			die("trigger '%s': unknown command '%s' at %u", cmd,
				&line[chari], cmdi);
	}
	if (ferror(stdin))
		die("trigger '%s': getline: %s", cmd, strerror(errno));

cleanup:
	free(line);
	if (kill(pid, SIGTERM))
		die("trigger '%s': kill: %s", cmd, strerror(errno));
	if (fclose(child))
		die("trigger '%s': fclose: %s", cmd, strerror(errno));
}
