...
main(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (errno)
			die("square: strtod '%s': %s", argv[1], strerror(errno));
		if ((*endptr != '\0') || (freq < 0.0))
			die("square: frequency must be nonnegative number, not '%s'", argv[1]);
	}
	...
	while (!feof(stdout)) {
		for (int bufi=0; bufi<BUFLEN; bufi++)
			if (freq) {
				buf[bufi] = phase < 0.5 ? MAXSAMPLE : MINSAMPLE;
		...
		if (ferror(stdout))
			die("square: fwrite: %s", strerror(errno));
	}
}
