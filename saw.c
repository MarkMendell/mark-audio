...
main(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (errno)
			die("saw: strtod '%s': %s", argv[1], strerror(errno));
		if ((*endptr != '\0') || (freq < 0.0))
			die("saw: frequency must be nonnegative number, not '%s'", argv[1]);
	}
	...
	while (!feof(stdout)) {
		for (int bufi=0; bufi<BUFLEN; bufi++)
			if (freq) {
				buf[bufi] = (MINSAMPLE+(phase*MAXSAMPLE))+(phase*MAXSAMPLE);
		...
		if (ferror(stdout))
			die("saw: fwrite: %s", strerror(errno));
	}
}
